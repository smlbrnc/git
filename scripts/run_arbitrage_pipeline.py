...
def main():
    record_pipeline_run("running", "Kontrol ve sorgular başlatıldı (Gemini, arbitraj, validation)")
    env = load_env()
    ...
    if not dep:
        record_pipeline_run("no_dependency", "Bağımsız piyasalar; arbitraj yok")
    ...
    if not has_arb:
        record_pipeline_run("no_arbitrage", "Fiyatlar arbitrajsız")
    ...
    if passed:
        ...
        record_pipeline_run("ok", f"Fırsat işlendi (mod: {mode})")
        ...
        record_pipeline_run("validation_failed", reason)
